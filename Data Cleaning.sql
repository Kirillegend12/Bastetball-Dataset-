...
-- Checking the Seasons with the small amount of played minutes 


SELECT RIGHT(season_id,4) as season, SUM(min) as total_min
FROM game
GROUP BY RIGHT(season_id,4)
HAVING SUM(min) < 20000;


-- Looking at the Data, which falls under the condition
with cte as
(
SELECT RIGHT(season_id,4) as season, SUM(min) as total_min
FROM game
...
FROM game 
WHERE RIGHT(season_id,4) >= 1980
AND game_id IS NOT NULL  -- Add this
  AND season_id IS NOT NULL; -- Add this
